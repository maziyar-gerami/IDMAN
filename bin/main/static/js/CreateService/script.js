    ...
    methods: {
      getUserInfo: function () {
        ...
        .then((res) => {
          ...
          axios.get(url + "/api/users/u/" + vm.username)
          .then((res) => {
      ...
      getGroups: function () {
        ...
        .then((res) => {
          ...
          for(var i = 0; i < vm.groups.length-1; ++i){
            vm.groupList += vm.groups[i].name + ',';
            vm.groups[i].name = "'" + vm.groups[i].name + "'";
          }
          vm.groupList += vm.groups[vm.groups.length-1].name;
          vm.groups[i].name = "'" + vm.groups[vm.groups.length-1].name + "'";
        });
      },
      addGroup: function (n) {
        n = n.split("'").join("");
