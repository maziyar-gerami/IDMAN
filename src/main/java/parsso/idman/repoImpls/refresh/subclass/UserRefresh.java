...
public class UserRefresh {
    ...
    @Autowired
    public UserRefresh(MongoTemplate mongoTemplate, LdapTemplate ldapTemplate, UniformLogger uniformLogger, UserRepo.UsersOp.Retrieve usersOpRetrieve,DashboardData dashboardData){
    ...
    public HttpStatus refresh(String doer) {
        ...
        for (User user : usersOpRetrieve.fullAttributes()) {
            ...
            try {
                ...
                userExtraInfo = mongoTemplate.findOne(new Query(new Criteria("_id").regex(user.get_id().toString(), "i")), UsersExtraInfo.class, Variables.col_usersExtraInfo);
        ...
        for (UsersExtraInfo usersExtraInfo : usersMongo) {
            List<UsersExtraInfo> usersExtraInfoList = ldapTemplate.search("ou=People," + BASE_DN, new EqualsFilter("uid", usersExtraInfo.get_id().toString()).encode(), searchControls, new SimpleUserAttributeMapper());
