package parsso.idman.repoImpls.role.subclass;
...
public class Update {
    ...
    public HttpStatus update(String doerID, String role, JSONObject users) {
        ...
        for (String userId : (List<String>) users.get("names")) {
            try {
                UsersExtraInfo usersExtraInfo =mongoTemplate.findOne(new Query(Criteria.where("_id").is(userId)), UsersExtraInfo.class, Variables.col_usersExtraInfo);
