package parsso.idman.repoImpls.skyroom;
...
import parsso.idman.repoImpls.skyroom.subclasses.LoginUrl;
import parsso.idman.repoImpls.skyroom.subclasses.RandomPass;
import parsso.idman.repoImpls.skyroom.subclasses.Register;
import parsso.idman.repoImpls.skyroom.subclasses.UserRooms;
...
public class SkyroomRepoImpl implements SkyroomRepo {
    ...
    public SkyRoom run(User user) throws IOException {
        ...
        int userId =  new Register(mongoTemplate,uniformLogger).register(RealName, new RandomPass().make(8), user.getDisplayName());
        ...
        if (userId == 0) {
            ...
            skyRoom = new SkyRoom(skyroomEnable, user.getUsersExtraInfo().getRole()
                    , new LoginUrl(mongoTemplate,uniformLogger).create(roomId, String.valueOf(userId), RealName), new UserRooms(mongoTemplate, uniformLogger).getGuestUrl(roomId));
