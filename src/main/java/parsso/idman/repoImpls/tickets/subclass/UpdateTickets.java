...
public class UpdateTickets {
    ...
    public UpdateTickets(MongoTemplate mongoTemplate, UniformLogger uniformLogger,UserRepo.UsersOp.Retrieve usersOpRetrieve){
        this.mongoTemplate = mongoTemplate;
        this.uniformLogger = uniformLogger;
    ...
    public HttpStatus update(String userId, String ticketId, Ticket newTicket) {
        ...
        try {
            ...
            uniformLogger.info(userId, new ReportMessage(Variables.MODEL_TICKETING, ticketId, "", Variables.ACTION_UPDATE, Variables.RESULT_SUCCESS, ticketToSave, ""));
            return HttpStatus.OK;
        } catch (Exception e) {
            e.printStackTrace();
            uniformLogger.warn(userId, new ReportMessage(Variables.MODEL_TICKETING, ticketId, "", Variables.ACTION_UPDATE, Variables.RESULT_FAILED, ticketToSave, "writing to MongoDB"));
    ...
    public HttpStatus status(String doer, int status, @RequestBody JSONObject jsonObject) {
        ...
        while (iterator.hasNext()) {
            ...
            try {
                ...
                uniformLogger.info(doer, new ReportMessage(Variables.MODEL_TICKETING, ticket.get_id(), Variables.ATTR_STATUS, Variables.ACTION_UPDATE, Variables.RESULT_SUCCESS, ""));

            } catch (Exception e) {
                uniformLogger.info(doer, new ReportMessage(Variables.MODEL_TICKETING, ticket.get_id(), Variables.ATTR_STATUS, Variables.ACTION_UPDATE, Variables.RESULT_FAILED, "Saving to MongoDB"));
