package parsso.idman.repoImpls.users.usersOprations.create;
...
import parsso.idman.repoImpls.users.usersOprations.create.helper.UsersCompare;
...
public class CreateUser extends Parameters implements UserRepo.UsersOp.Create  {
    ...
    @Autowired
    public CreateUser(LdapTemplate ldapTemplate, MongoTemplate mongoTemplate, UniformLogger uniformLogger, UserRepo.UsersOp.Retrieve userOpRetrieve,
        ...
        try {
            if (user == null || user.get_id().toString() == null) {
                ...
                if (new GroupsChecks(retrieveGroup).checkGroup(p.getMemberOf())) {
                    ...
                    //create user in ldap
                    ldapTemplate.bind(new BuildDnUser(BASE_DN).buildDn(p.get_id().toString()), null, buildAttributes.build(p));

                    if (p.getStatus() != null)
                        if (p.getStatus().equals("disable"))
                            operations.disable(doerID, p.get_id().toString());

                    mongoTemplate.save(new UsersExtraInfo(p, p.getPhoto(), p.isUnDeletable()), Variables.col_usersExtraInfo);
