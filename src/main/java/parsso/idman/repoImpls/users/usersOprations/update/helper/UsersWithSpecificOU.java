...
public class UsersWithSpecificOU {
    ...
    public void updateUsersWithSpecificOU(String doerID, String old_ou, String new_ou) {
        ...
        try {
            ...
            for (User user : getUsersOfOu(old_ou)) {
                ...
                DirContextOperations context = buildAttributes.buildAttributes(doerID, user.get_id().toString(), user, new BuildDnUser(BASE_DN).buildDn(user.get_id().toString()));
    ...
    public List<User> getUsersOfOu(String ou) {
        ...
        List<User> users = ldapTemplate.search("ou=People," + BASE_DN, andFilter.toString(), searchControls, new UserAttributeMapper(mongoTemplate)
        );
