...
public class UpdatePubMessage {
    ...
    public HttpStatus editPubicMessage(String doer, PublicMessage message) {
        ...
        PublicMessage publicMessage = mongoTemplate.findOne(new Query(Criteria.where("_id").is(message.getMessageId())), PublicMessage.class, Variables.col_publicMessage);
        message.set_id(Objects.requireNonNull(publicMessage).get_id());
        try {
            mongoTemplate.save(message, Variables.col_publicMessage);
            uniformLogger.info(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, message.getMessageId(), "", "update", Variables.RESULT_SUCCESS, ""));

            return HttpStatus.OK;
        } catch (Exception e) {
            e.printStackTrace();
            uniformLogger.warn(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, message.getMessageId(), "", Variables.ACTION_UPDATE, Variables.RESULT_FAILED, "Writing to mongo"));
