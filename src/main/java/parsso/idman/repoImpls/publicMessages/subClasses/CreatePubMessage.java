...
public class CreatePubMessage {
    ...
    public HttpStatus postPubicMessage(String doer, PublicMessage message) {
        ...
        try {
            ...
            uniformLogger.info(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, messageToSave.getMessageId(), "", Variables.ACTION_CREATE, Variables.RESULT_SUCCESS, ""));

            return HttpStatus.OK;

        } catch (Exception e) {
            e.printStackTrace();
            uniformLogger.warn(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, message.getMessageId(), "", Variables.ACTION_CREATE, Variables.RESULT_FAILED, "Writing to mongo"));
