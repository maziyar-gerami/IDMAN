...
public class CreateTickets {
  ...
  public HttpStatus sendTicket(Ticket ticket, String userid) {
    try {
      ...
      uniformLogger.info(userid, new ReportMessage(Variables.MODEL_TICKETING, ticketToSave.get_id(), "",
          Variables.ACTION_CREATE, Variables.RESULT_SUCCESS, ""));

      return HttpStatus.OK;
    } catch (Exception e) {
      uniformLogger.error(userid, new ReportMessage(Variables.MODEL_TICKETING, ticket.get_id(), "",
  ...
  public HttpStatus reply(String ticketID, String userid, Ticket replyTicket) {
    ...
    if (ticket.getStatus() < st && st == 2) {
      messages.add(new Message(usersOpRetrieve.retrieveUsers(userid), usersOpRetrieve.retrieveUsers(to),
          replyTicket.getMessage()));
    ...
    try {
      mongoTemplate.save(ticketToSave, Variables.col_tickets);
      uniformLogger.info(userid, new ReportMessage(Variables.MODEL_TICKETING, ticketToSave.get_id(), "", "reply",
          Variables.RESULT_SUCCESS, ""));
    } catch (Exception e) {

      e.printStackTrace();
      uniformLogger.warn(userid, new ReportMessage(Variables.MODEL_TICKETING, ticketToSave.get_id(), "", "reply",
          Variables.RESULT_FAILED, "writing to MongoDB"));
