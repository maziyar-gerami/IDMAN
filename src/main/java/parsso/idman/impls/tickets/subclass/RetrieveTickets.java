...
import parsso.idman.impls.tickets.helper.TicketsCount;
...
public class RetrieveTickets {
  ...
  public ListTickets sent(String userId, String page, String count, String date) {
    ...
    if (!date.equals("")) {
      String time = new Time(Integer.parseInt(date.substring(4)), Integer.parseInt(date.substring(2, 4)),
          Integer.parseInt(date.substring(0, 2))).toStringDate();
      ...
      long eventStartDate = OffsetDateTime.parse(timeStart).atZoneSameInstant(ZoneId.of(Variables.ZONE))
          .toEpochSecond() * 1000;
      long eventEndDate = OffsetDateTime.parse(timeEnd).atZoneSameInstant(ZoneId.of(Variables.ZONE))
          .toEpochSecond() * 1000;
  ...
  public ListTickets recieved(String userId, String page, String count, String from, String ticketId, String date) {
    ...
    Query query = new Query(Criteria.where("to").is(userId).and("status").is(1))
        .with(Sort.by(Sort.Direction.DESC, "creatinTime")).skip(skip).limit(limit);
    ...
    int size = (int) mongoTemplate.count(query, Variables.col_tickets);
    return new ListTickets((int) mongoTemplate.count(query, Variables.col_tickets), ticketList,
        (int) Math.ceil((double) size / (double) limit));
  }
  ...
  public ListTickets retrieve(String doer, String cat, String subCat, String status, String page, String count,
    ...
    query.addCriteria(Criteria.where("deleteFor").ne(doer)).skip(skip).limit(limit)
        .with(Sort.by(Sort.Direction.DESC, "creationTime"));
  ...
  public ListTickets archived(String cat, String subCat, String page, String count, String from, String ticketId,
      String date) {
