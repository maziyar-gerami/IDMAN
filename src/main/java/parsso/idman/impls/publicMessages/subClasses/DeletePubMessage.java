  ...
  public HttpStatus deletePubicMessage(String doer, JSONObject jsonObject) {
    ...
    if (!iterator.hasNext()) {
      try {
        ...
        uniformLogger.info(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, "All", "",
            Variables.ACTION_DELETE, Variables.RESULT_SUCCESS, ""));

      } catch (Exception e) {

        e.printStackTrace();
        uniformLogger.warn(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, "All", "",
            Variables.ACTION_DELETE, Variables.RESULT_FAILED, ""));
    ...
    while (iterator.hasNext()) {
      ...
      try {
        ...
        e.printStackTrace();
        uniformLogger.warn(doer, new ReportMessage(Variables.MODEL_PUBICMESSAGE, next, "",
            Variables.ACTION_DELETE, Variables.RESULT_FAILED, "Writing to mongo"));
      }
