...
public class AuthenticatorController {
    ...
    public ResponseEntity<Response> retrieveDevices(@RequestParam(value = "username", defaultValue = "") String username,
        ...
        return new ResponseEntity<>(new Response(authenticatorRepo.retrieve(username, name, Integer.parseInt(page), Integer.parseInt(count)),Variables.MODEL_USER,HttpStatus.OK.value(), lang), HttpStatus.OK);
    }
    ...
    public ResponseEntity<Response> deleteBuUsername(HttpServletRequest request, @RequestParam(value = "username", defaultValue = "") String username,
        ...
        if (username.equals("") && deviceName.equals(""))
            ...
            return new ResponseEntity<>(new Response(null, Variables.MODEL_AUTHENTICATOR, authenticatorRepo
                    .deleteByUsernameAndDeviceName(username, deviceName, request.getUserPrincipal().getName()).value(),lang), HttpStatus.OK);
        else if (!username.equals(""))
            return new ResponseEntity<>(new Response(null, Variables.MODEL_AUTHENTICATOR, authenticatorRepo
                    .deleteByUsername(username, request.getUserPrincipal().getName()).value(),lang), HttpStatus.OK);
        else
            return new ResponseEntity<>(new Response(null, Variables.MODEL_AUTHENTICATOR, authenticatorRepo.
                    deleteByDeviceName(deviceName, request.getUserPrincipal().getName()).value(),lang), HttpStatus.OK);
    }
