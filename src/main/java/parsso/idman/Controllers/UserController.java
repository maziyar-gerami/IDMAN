...
public class UserController {
    ...
    public void getImage(HttpServletResponse response, HttpServletRequest request) throws IOException {
        ...
        File file = new File(uploadedFilesPath+user.getPhotoPath());
    ...
    public RedirectView uploaduProfilePic(@RequestParam("file") MultipartFile file,
        ...
        try {
            ...
            User user = userRepo.retrieveUser(principal.getName());
            user.setPhotoPath(StTime + file.getOriginalFilename());
    ...
    {
        Principal principal = request.getUserPrincipal();
        return new ResponseEntity<HttpStatus>(userRepo.changePassword(principal.getName(),currentPassword,newPassword), HttpStatus.FOUND);
