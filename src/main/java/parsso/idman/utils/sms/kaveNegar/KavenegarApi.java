...
public class KavenegarApi {
  ...
  public List<SendResult> send(String sender, List<String> receptors, String message, MessageType type, long date,
      List<String> localIds) throws BaseException {
    ...
    JsonArray entry = execute(getApiPath("sms/send"),
        ...
        "type", type.getValue(),
        "date", date,
        "localid", localIds == null ? "" : StringUtils.join(",", localIds)).getAsJsonArray();
  ...
  public List<SendResult> sendArray(List<String> senders, List<String> receptors, List<String> messages,
      List<MessageType> types, long date, List<String> localIds) throws BaseException {
    ...
    JsonArray entry = execute(path, "sender", jsonSenders.toString(), "receptor", jsonReceptors.toString(),
        "message", jsonMessages.toString(), "date", date, "type", jsonTypes.toString(), "localids", localIds)
        .getAsJsonArray();
    List<SendResult> list = new ArrayList<>();
    for (int i = 0; i < entry.size(); i++) {
      JsonObject json = entry.get(i).getAsJsonObject();
  ...
  public List<SendResult> sendArray(List<String> senders, List<String> receptors, List<String> messages,
      String localId) throws BaseException {
  ...
  public List<SendResult> sendArray(List<String> senders, List<String> receptors, List<String> messages,
      List<MessageType> types, long date, String localId) throws BaseException {
    List<String> localIds = new ArrayList<>();
    for (int i = 0; i < receptors.size(); i++) {
      localIds.add(localId);
    }
  ...
  public List<SendResult> sendArray(String sender, List<String> receptors, List<String> messages, String localId)
      throws BaseException {
    List<MessageType> types = new ArrayList<>();
  ...
  public CountOutboxResult countOutbox(long startDate, long endDate, int status) throws BaseException {
    ...
    JsonObject entry = execute(path, "startdate", startDate, "enddate", endDate, "status", status).getAsJsonArray()
  ...
  public CountInboxResult countInbox(long startDate, long endDate, String lineNumber, int isRead)
    ...
    JsonObject entry = execute(path, "startdate", startDate, "enddate", endDate, "linenumber", lineNumber, "isread",
        isRead).getAsJsonArray().get(0).getAsJsonObject();
  ...
  public List<SendResult> sendByPostalCode(long postalCode, String sender, String message, long mciStartIndex,
      long mciCount, long mtnStartIndex, long mtnCount, long date) throws BaseException {
    String path = getApiPath("sms/sendpostalcode");
    JsonArray entry = execute(path, "postalcode", postalCode, "sender", sender, "message", message, "mcistartIndex",
        mciStartIndex, "mcicount", mciCount, "mtnstartindex", mtnStartIndex, "mtncount", mtnCount, "date", date)
        .getAsJsonArray();
    List<SendResult> list = new ArrayList<>();
    for (int i = 0; i < entry.size(); i++) {
      JsonObject json = entry.get(i).getAsJsonObject();
  ...
  public List<SendResult> sendByPostalCode(long postalCode, String sender, String message, long mciStartIndex,
      long mciCount, long mtnStartIndex, long mtnCount) throws BaseException {
  ...
  public AccountConfigResult accountConfig(String apiLogs, String dailyReport, String debugMode, String defaultSender,
      int minCreditAlarm, String resendFailed) throws BaseException {
    String path = getApiPath("account/config");
    JsonObject json = execute(path, "apilogs", dailyReport, "dailyreport", debugMode, "debugmode", debugMode,
        "defaultsender", defaultSender, "mincreditalarm", minCreditAlarm, "mincreditalarm", resendFailed)
        .getAsJsonObject();
  ...
  public SendResult verifyLookup(String receptor, String token, String token2, String token3, String template)
    ...
    JsonArray array = execute(path, "receptor", receptor, "token", token, "token2", token2, "token3", token3,
        "template", template).getAsJsonArray();
    JsonObject json = array.get(0).getAsJsonObject();
    return new SendResult(json);
  }

  public SendResult verifyLookup(String receptor, String token, String token2, String token3, String template,
      List<PairValue> params) throws BaseException {
    String path = getApiPath("verify/lookup");
    String token10 = null;
    String token20 = null;
    ...
    JsonArray array = execute(path, "receptor", receptor, "token", token, "token2", token2, "token3", token3,
        "template", template, "token10", token10, "token20", token20).getAsJsonArray();
    JsonObject json = array.get(0).getAsJsonObject();
    return new SendResult(json);
  }
