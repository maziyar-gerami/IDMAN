...
public class BuildAttributes {
  ...
  public Attributes build(User p) {
    ...
    String employeeNumber = new String(p.getEmployeeNumber().getBytes(StandardCharsets.UTF_8),
    ...
    String description = new String(p.getDescription().trim().getBytes(StandardCharsets.UTF_8),
        StandardCharsets.UTF_8);
  ...
  public DirContextOperations buildAttributes(String doerID, String uid, User p, Name dn) {
    ...
    if (p.getFirstName() != null)
      if (!p.getFirstName().equals("")) {
        String givenName = new String(p.getFirstName().trim().getBytes(StandardCharsets.UTF_8),
            StandardCharsets.UTF_8);
    ...
    if (p.getDisplayName() != null)
      if (!p.getDisplayName().equals("")) {
        String displayName = new String(p.getDisplayName().getBytes(StandardCharsets.UTF_8),
            StandardCharsets.UTF_8);
    ...
    if (p.getMemberOf() != null) {
      if (p.getMemberOf().size() != 0 && !p.getMemberOf().get(0).equals("")) {
        for (int i = 0; i < p.getMemberOf().size(); i++) {
          if (old.getMemberOf() == null && i == 0)
            context.setAttributeValue("ou", p.getMemberOf().get(i));
          else
            context.addAttributeValue("ou", p.getMemberOf().get(i));
        }
