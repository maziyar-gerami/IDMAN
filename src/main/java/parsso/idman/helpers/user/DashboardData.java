...
public class DashboardData {
  ...
  public void updateDashboardData() throws InterruptedException {
    ...
    Thread userData = new Thread(() -> {
      ...
      int disabled = (int) mongoTemplate.count(new Query(Criteria.where("status").is("disable")),
          UsersExtraInfo.class, Variables.col_usersExtraInfo);
      int locked = (int) mongoTemplate.count(new Query(Criteria.where("status").is("lock")),
          UsersExtraInfo.class, Variables.col_usersExtraInfo);
      int temp = users - locked - disabled;
      int active = Math.min((temp), users);

      fUsers = new Dashboard.Users(users, active, disabled, locked);
