...
public class SimpleUserAttributeMapper implements AttributesMapper<UsersExtraInfo> {
  ...
  public UsersExtraInfo mapFromAttributes(Attributes attributes) throws NamingException {
    ...
    user.setDisplayName(
        null != attributes.get("displayName") ? attributes.get("displayName").get().toString() : null);
    ...
    // Long.valueOf(attributes.get("pwdChangedTime").get().toString().substring(0,
    // 14)) :
    // Long.valueOf(attributes.get("createtimestamp").get().toString().substring(0,
    // 14)));
